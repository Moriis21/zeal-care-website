...
const inputClass = "w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#09609A]/30 focus:border-[#09609A]";
    ...
    <div>
      ...
      {multiline ? (
        <textarea rows={rows} value={value} onChange={(e) => onChange(e.target.value)}
          className="w-full border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-[#09609A]/30 focus:border-[#09609A] transition-colors resize-none" />
      ) : (
        <input value={value} onChange={(e) => onChange(e.target.value)}
          className="w-full border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-[#09609A]/30 focus:border-[#09609A] transition-colors" />
      )}
    </div>
  );
}
    ...
    <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
      <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
        ...
        <button onClick={onSave} disabled={saving}
          className="flex items-center gap-2 bg-[#09609A] text-white px-4 py-2 rounded-xl text-sm font-semibold hover:bg-[#09609A]/90 transition-colors disabled:opacity-60">
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 space-y-2">
          ...
          <button type="button" onClick={() => fileRef.current?.click()} disabled={uploading}
            className="flex items-center gap-1.5 bg-[#09609A] text-white px-3 py-1.5 rounded-lg text-xs font-semibold hover:bg-[#09609A]/90 transition-colors disabled:opacity-60 w-full justify-center">
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-3">
        ...
        <button onClick={startAdd} className="flex items-center gap-1.5 text-xs font-semibold text-[#09609A] hover:text-[#09609A]/80 transition-colors">
      ...
      <div className="border border-slate-200 rounded-xl overflow-hidden divide-y divide-slate-100">
            ...
            {editIdx === idx ? (
              <div className="p-4 bg-blue-50 space-y-3">
                ...
                <div className="flex gap-2">
                  <button onClick={saveEdit} className="flex items-center gap-1.5 bg-[#09609A] text-white px-3 py-1.5 rounded-lg text-xs font-semibold"><Check className="w-3 h-3" /> Save</button>
              ...
              <div className="flex items-center gap-3 px-4 py-3 hover:bg-slate-50">
                ...
                <div className="flex gap-1.5 flex-shrink-0">
                  <button onClick={() => startEdit(idx)} className="p-1.5 text-slate-400 hover:text-[#09609A] hover:bg-blue-50 rounded-lg transition-colors"><Pencil className="w-3.5 h-3.5" /></button>
        ...
        {addingNew && (
          <div className="p-4 bg-blue-50 space-y-3">
            {renderForm(draft, setDraft)}
            <div className="flex gap-2">
              <button onClick={saveAdd} className="flex items-center gap-1.5 bg-[#09609A] text-white px-3 py-1.5 rounded-lg text-xs font-semibold"><Check className="w-3 h-3" /> Add</button>
              <button onClick={cancelEdit} className="flex items-center gap-1.5 text-slate-500 px-3 py-1.5 rounded-lg text-xs font-semibold hover:bg-slate-100"><X className="w-3 h-3" /> Cancel</button>
            </div>
          </div>
        )}
    ...
    return (
      <AdminLayout>
        <div className="flex items-center justify-center h-64">
          <Loader2 className="w-8 h-8 animate-spin text-[#09609A]" />
        </div>
      </AdminLayout>
    );
  }
          ...
          {TABS.map(({ id, label, icon: Icon }) => (
            <button key={id} onClick={() => setActiveTab(id)}
              className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold transition-all ${
                activeTab === id
                  ? "bg-[#09609A] text-white shadow-sm"
                    ...
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-full overflow-hidden bg-slate-100 flex-shrink-0 border border-slate-200">
                        {tm.img ? (
                          ...
                          <div className="w-full h-full flex items-center justify-center text-slate-400 text-sm font-bold bg-[#061A32] text-[#FBD308]">
                            {tm.name?.[0] ?? "?"}
                          </div>
                        )}
                      </div>
                  ...
                  return (
                    <div className="flex items-center gap-3">
                      {bm.img ? (
                        ...
                        <div className="w-10 h-10 rounded-full bg-[#061A32] flex items-center justify-center text-[#FBD308] text-sm font-bold flex-shrink-0">
                          {bm.name?.[0] ?? "?"}
                        </div>
                      )}
                      <div>
                  ...
                  return (
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        ...
                        <span className="inline-block text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full bg-[#09609A]/10 text-[#09609A] mt-1">
                          {photo.category}
                        </span>
                      </div>
                    </div>
