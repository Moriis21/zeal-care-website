...
type Category = "All" | "Programs & Education" | "Children's Stories" | "Community" | "Media & Events" | "Team";
...
const CATEGORIES: Category[] = ["All", "Programs & Education", "Children's Stories", "Community", "Media & Events", "Team"];
...
export default function GalleryPage() {
  ...
  const counts: Record<Category, number> = {
    All: PHOTOS.length,
    "Programs & Education": PHOTOS.filter((p) => p.category === "Programs & Education").length,
    "Children's Stories": PHOTOS.filter((p) => p.category === "Children's Stories").length,
    Community: PHOTOS.filter((p) => p.category === "Community").length,
    "Media & Events": PHOTOS.filter((p) => p.category === "Media & Events").length,
  ...
  return (
    <>
        ...
        <div className="container mx-auto px-4 relative">
          ...
          >
            ...
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-black text-white leading-tight mb-5">
              Moments of{" "}
              <span className="text-[#F5C619] italic">Impact</span>
            </h1>
            ...
            <div className="flex items-center gap-6 mt-8 pt-8 border-t border-white/10">
              <div>
                <p className="text-2xl font-black text-[#F5C619]">{PHOTOS.length}</p>
                <p className="text-white/40 text-xs font-semibold uppercase tracking-wider mt-0.5">Photos</p>
              </div>
              <div className="w-px h-8 bg-white/10" />
              <div>
                <p className="text-2xl font-black text-[#F5C619]">{CATEGORIES.length - 1}</p>
      ...
      {/* Filter bar */}
      <section className="bg-white border-b border-slate-100 sticky top-[0px] z-30 shadow-sm">
            ...
            {CATEGORIES.map((cat) => (
              <button
                ...
                className={`flex-shrink-0 flex items-center gap-2 px-4 py-2 rounded-full text-sm font-semibold transition-all duration-200 ${
                  activeCategory === cat
                    ? "bg-[#061A32] text-white shadow-md"
              ...
              {filtered.map((photo, idx) => (
                ...
                >
                  ...
                  <div className="absolute inset-0 bg-[#061A32]/0 group-hover:bg-[#061A32]/55 transition-all duration-300 flex flex-col justify-end p-4">
                    <div className="opacity-0 group-hover:opacity-100 transition-opacity duration-300 translate-y-2 group-hover:translate-y-0 transition-transform">
                      <span
